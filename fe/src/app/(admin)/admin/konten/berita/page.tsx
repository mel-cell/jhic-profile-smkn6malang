...
const BeritaCard = ({ data, onDelete }: { data: any; onDelete: (id: string) => void }) => (
    ...
    <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden relative group">
        ...
        <div className="h-40 w-full overflow-hidden relative">
            <img 
                src={data.imagePath || '/placeholder-berita.jpg'} 
                alt={data.judul} 
        ...
        <div className="p-3">
            <h3 className="text-base font-bold text-gray-900 mb-3 leading-tight">{data.judul}</h3>
            ...
            <div className="flex justify-between items-center text-sm">
                ...
                <div className="flex space-x-1.5">
                    <Link href={`/admin/konten/berita/edit/${data.id}`} title="Edit" className="p-1 text-blue-500 hover:text-blue-700 bg-blue-50 rounded-full">
    ...
    const handleDelete = async (id: string) => {
        if (confirm('Apakah Anda yakin ingin menghapus berita ini?')) {
            try {
                ...
                if (response.success) {
                    setBeritaData(prev => prev.filter(item => item.id !== id));
                    alert('Berita berhasil dihapus!');
                } else {
                    alert('Gagal menghapus berita: ' + response.error);
                }
            } catch (error) {
        ...
        <div className="p-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                {filteredData.length > 0 ? (
                    filteredData.map((item: any) => <BeritaCard key={item.id} data={item} onDelete={handleDelete} />)
                ) : (
